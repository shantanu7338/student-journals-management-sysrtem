...
const Teacherjournals=()=>
{
    ...
    {
        e.preventDefault();
        await axios.post('http://localhost:5000/check',search).then((res)=>
        {
    ...
    const tbody=(val)=>
    {
        return (
            <tr className="hover:bg-yellow-200">
                <td className="border border-black border-collapse py-2">{val.student_id}</td>
                <td className="border border-black border-collapse py-2">{val.student_name}</td>
    ...
    return (
        <>
            ...
            <div className="w-full border border-black py-2">
                <form className="w-5/6 mx-auto px-20" method="POST" onSubmit={search1}>
                    <h2 className="font-bold text-2xl text-center uppercase my-2">Check Assigments</h2>
                    <label>Course</label>&nbsp;
                    ...
                    <label>title</label>&nbsp;
                    <input type="text" name="title" onChange={input} className="shadow appearance-none border rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />&nbsp;&nbsp;
                ...
                <table className="w-full capitalize border border-black border-collapse text-center">
                    <thead className="bg-black text-white">
                        <tr>
                            ...
                            <th className="border border-black border-collapse py-2">submission date</th>
